...
public class LoadService<T> {
    ...
    LoadService(Convertor<T> convertor, TargetContext targetContext, Callback.OnReloadListener onReloadListener,
        ...
        View oldContentView = targetContext.getOldContentView();
        ViewGroup.LayoutParams oldLayoutParams = oldContentView.getLayoutParams();
        loadLayout = new LoadLayout(context, onReloadListener);
        loadLayout.setupSuccessLayout(new SuccessCallback(oldContentView, context,
                onReloadListener));
